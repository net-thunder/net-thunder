...
@Slf4j
public abstract class ICEElectionProtocol implements ElectionProtocol {

    private Config config;
    private P2pClient p2pClient;
    private RelayClient relayClient;

    public ICEElectionProtocol(Config config,
                               P2pClient p2pClient,
    ...
    @Override
    public CompletableFuture<DataTransport> createOffer(SDWanProtos.NodeInfo nodeInfo) {
        if (config.getShowElectionLog()) {
            log.info("createOffer: src={}, dst={}", getLocalVip(), nodeInfo.getVip());
        }
        ...
        String reqId = ShortUUID.gen();
        return sendOffer(reqId, p2pOffer, 3 * config.getElectionTimeout())
                .thenApply(resp -> {
                    try {
    ...
    @Override
    public CompletableFuture<DataTransport> createAnswer(String reqId, SDWanProtos.P2pOffer p2pOffer) {
        if (config.getShowElectionLog()) {
            log.info("createAnswer: src={}, dst={}", p2pOffer.getDstVIP(), p2pOffer.getSrcVIP());
        }
    ...
    protected abstract CompletableFuture<SDWanProtos.P2pAnswer> sendOffer(String reqId, SDWanProtos.P2pOffer p2pOffer, long timeout);
